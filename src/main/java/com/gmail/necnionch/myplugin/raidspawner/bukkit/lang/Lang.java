...
public enum Lang {
    ...
    COMmAND_SETNORAIDDAY_NOT_SPECIFIED_DAYS("&c襲撃イベントを休止する日数を指定してください"),
    COMMAND_SETNORAIDDAY_INVALID_DAYS("&c休止する日数を数値で指定してください"),
    COMMAND_SETNORAIDDAY_INVALID_DAYS_OLD("&c指定された数値はすでに経過しています"),
    COMMAND_SETNORAIDDAYS_DONE("&6%1$s Land の襲撃イベントを %2$s まで休止するように設定しました"),
    COMMAND_SETNORAIDDAYS_ALL_DONE("&6襲撃イベントを %1$s まで休止するように設定しました"),
    COMMAND_SETNORAIDDAYS_ALREADY("&cすでに %1$s Land の襲撃イベントが %2$s まで休止に設定されています"),
    COMMAND_SETNORAIDDAYS_ALREADY_ALL("&cすでに襲撃イベントが %1$s まで休止に設定されています"),
    COMMAND_UNSETNORAIDDAYS_DONE("&6%1$s Land の襲撃イベントの休止設定を解除しました"),
    COMMAND_UNSETNORAIDDAYS_ALL_DONE("&6襲撃イベントの休止設定を解除しました"),
    COMMAND_UNSETNORAIDDAYS_ALREADY("&c%1$s Land の襲撃イベントは休止設定にされていません"),
    COMMAND_UNSETNORAIDDAYS_ALREADY_ALL("&c襲撃イベントは休止設定にされていません"),
